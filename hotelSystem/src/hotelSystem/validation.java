...
public class validation {
	...
	public static String validatePasswords(String password, Scanner scan){
        ...
        while(passwordTime != 'y'){
            System.out.println("Your password is invalid. Remember, your password must be at least 8 characters but no more than 20, and must contain at least one uppercase letter, one lowercase letter, one digit, and one special character.");
            password = scan.next();
            if (password.matches(passwordReg)){
                passwordTime = 'y';
            }
